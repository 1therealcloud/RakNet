...
{
    if (data == 0 || encodedData == 0 || length < 0 || length > MAXIMUM_MTU_SIZE)
        return -1;

    unsigned char checksum = 0;

    for (int index = 0; index < length; ++index)
        checksum ^= static_cast<unsigned char>(data[index]) & 0xAA;

    sampDatagramBuffer[0] = checksum;

    for (int index = 0; index < length; ++index)
    {
        unsigned char value =
            sampDatagramEncryptionTable[static_cast<unsigned char>(data[index])];

        if ((index & 1) != 0)
            value ^= portKey;

        sampDatagramBuffer[index + 1] = value;
    }

    *encodedData = reinterpret_cast<const char *>(sampDatagramBuffer);
    return length + 1;
}
